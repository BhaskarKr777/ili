...
Input formats:
  ...
  Scheduled:  "title | message | HH:MM"
              "title | message | in X minutes"
              "title | message | now + X minutes"
"""
...
class NotificationTool(BaseTool):
    ...
    def _schedule(self, title: str, message: str, schedule_str: str) -> str:
        ...
        # ── "now + X minutes/hours" ────────────────────────────────────────────
        match = re.match(r'now\s*\+\s*(\d+)\s*(minute|minutes|min|hour|hours|hr|second|seconds|sec)', schedule_str)
        if match:
        ...
        if delay_seconds is None:
            return (
                ...
                "Use 'HH:MM' (e.g. 14:30 or 2:30 pm), 'in X minutes', or 'now + X minutes'."
            )
        ...
        return f"Notification scheduled for {fire_at_str} — \"{title}: {message}\""